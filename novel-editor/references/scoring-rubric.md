...
- Concrete improvement suggestions, specific enough to be executed without asking a follow-up question. Give **three for the single most damaging weakness on each axis, and one each** for the rest. The old "three per item" rule produced fifteen-plus suggestions that no author can act on in one revision cycle, and volume itself hides priority — the point is a revision that actually happens, not an exhaustive list.
...
## 軸別評価
### 構成
（達成条件の判定と引用 → 上限規則の適用有無 → 3部構成の根拠 → **点数：X/10** → 弱点と改善案（3件以上）→ 機能している点）
### キャラクター
### 世界観
### 感情設計
### 牽引力
